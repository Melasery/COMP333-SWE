...
In addition to coming up with tests for the code, another way AI could be used is to test for things that don't necessarily have a provably correct or incorrect answer. For example, our feature for the project is a song suggestion algorithm, but the question of what song is most “similar” to another isn't something objective that can be determined with traditional tests. However, a generative AI model (like GPT) could be prompted to estimate which songs should be similar and then used as a reference to evaluate whether our algorithm produces reasonable results. This enables testing that’s more nuanced and human-aligned, especially for subjective domains like music taste.
For our final feature, we also integrated OpenAI’s GPT API directly into the app, not just for testing, but as a core user-facing tool. We built an AI-powered music chatbot that users can interact with conversationally to get personalized song suggestions based on their mood, vibe, or activity. This demonstrates how generative AI can be both a tool for testing fuzzy logic and a feature that expands the capabilities of the app.
If we had more time, we could also have used generative AI tools to generate PHPUnit test cases automatically, e.g., by describing a test scenario to GPT and asking it to write assertions in PHP. In fact, GPT was used during development to help write codes, debug backend logic, explain error messages, and propose improvements to REST endpoint handling.
